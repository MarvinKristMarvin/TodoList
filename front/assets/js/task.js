const taskManager = {
  ...
  fetchAndInsertTasksFromApi: async function (event) {
    ...
    response = await fetch(`${taskManager.apiEndpoint}/tasks`);
    if (!response.ok) {
  ...
  insertTaskInHtml: function (taskData) {
    ...
    newTask
      ...
      .addEventListener("click", taskManager.handleDeleteButton);

    newTask
      .querySelector(".task__edit")
      .addEventListener("click", taskManager.handleEditButton);

    newTask
      .querySelector(".task__edit-form")
      .addEventListener("submit", taskManager.handleEditForm);
